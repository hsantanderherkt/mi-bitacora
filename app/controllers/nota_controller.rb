class NotaController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @notum.update(notum_params)
        format.html { redirect_to @notum, notice: "Notum was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to nota_path, notice: "Notum was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_notum
      @notum = Notum.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def notum_params
      params.expect(notum: [ :titulo, :contenido ])
    end
end
